...
from utils.cloudflaresolving.utils import extract_domain, compressToEncodedURIComponent, check_for_captcha
import re, base64, time, requests
import importlib
from bs4 import BeautifulSoup
from utils.cloudflaresolving.exceptions import SolvingError
...
class CF_2():
    def __init__(self, adapter, original, key, captcha=False, debug=False):
        # Config vars
        self.script = "https://{}/cdn-cgi/{}"
        self.script_source = original.text.split('.src="/cdn-cgi/')[1].split('"')[0]
    ...
    def solve(self):
        ...
        while True:
            ...
            if self.solveRetries == self.solveMaxRetries:
                ...
                # Fetching CF script
                script = self.script.format(self.domain, self.script_source)
            ...
            if self.initPayloadRetries == self.initPayloadMaxRetries:
                ...
                try:
                    ...
                    challenge_payload = self.s.post("https://{}/cf-a/ov1/p1".format(self.api_domain),
                                                      params=self.auth_params, json=payload,
    ...
    def solve_payload(self, img = None,rerun_payload = None):
        ...
        while True:
            ...
            if self.fetchingChallengeRetries == self.fetchingChallengeMaxRetries:
                ...
                try:
                    ...
                    cc = self.s.post("https://{}/cf-a/ov1/p2".format(self.api_domain),
                                       params=self.auth_params, json=payload, timeout=self.timeOut,verify=False)
                    cc = cc.json()
    ...
    def get_image(self, img,rerun_payload = None):
        ...
        while True:
            if self.debug:
                print(f"Loading image. ({self.submitChallengeRetries}/{self.submitChallengeMaxRetries})")

            if self.submitChallengeRetries == self.submitChallengeMaxRetries:
                raise Exception(f"Loading image failed after {self.submitChallengeMaxRetries} retries.")
            else:
                self.submitChallengeRetries += 1

                try:
                    self.mainpayload_response = self.adapter.get(img, timeout=self.timeOut)
                    img = base64.b64encode(self.mainpayload_response.content).decode()

                except Exception as e:
                    if self.debug:
                        print(f"Loading image error: {str(e)}")
                    time.sleep(self.errorDelay)
                    continue
                else:
                    self.submitChallengeRetries = 0
                    ...
                    return self.solve_payload(img,rerun_payload)
    ...
    def handle_final_api(self):
        ...
        if self.final_api["status"] == "reload":
            self.captcha_response_api = self.final_api
        ...
        while True:
            ...
            if self.rerunRetries == self.rerunMaxRetries:
                ...
                try:
                    ...
                    alternative = self.s.post("https://{}/cf-a/ov1/p2".format(self.api_domain),
                                                params=self.auth_params, json=payload, timeout=self.timeOut,verify=False)
                    ...
                    if self.result == "img":
                        img = alternative["img"]
                        return self.get_image(img,payload)
                    else:
                        return self.send_main_payload()
        ...
        while True:
            ...
            if self.captchaRetries == self.captchaMaxRetries:
                ...
                try:
                    ...
                    ff = self.s.post("https://{}/cf-a/ov1/cap1".format(self.api_domain),
                                       params=self.auth_params, json=payload, timeout=self.timeOut,verify=False)
