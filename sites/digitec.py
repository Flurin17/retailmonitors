...
class monitor:
    ...
    def getProducts(self):
        while True:
            try:
                r = self.session.post(
                url='https://www.digitec.ch/api/graphql/get-tag-filter-products',
                headers={
                ...
                json = [
                    {
                    'operationName': 'GET_TAG_FILTER_PRODUCTS',
                    'variables': {
                        'tagId': 835,
                        'offset': 0,
                        'limit': 84,
